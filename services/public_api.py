...
class GetTokenRequest:
    code: str
    redirect_uri: str = "https://flask-tt-g5eenwndxa-rj.a.run.app/callback"
...
class ListWorkoutsRequest:
    def execute(self, list_workouts_url: str, access_token: str) -> ListWorkoutsResponse:
        response: requests.Response = requests.get(
            list_workouts_url,
            headers={"Authorization": f"Bearer {access_token}"},
            timeout=120
        ...
        return None if not response.ok else ListWorkoutsResponse(
            data = json.dumps(response.json(), indent=4),
            status_code = response.status_code,
            message = response.raw
        )
